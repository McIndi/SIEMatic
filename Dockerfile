...
USER appuser
EXPOSE 8000
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 CMD ["python", "-c", "import os, ssl, sys, urllib.request; tls_enabled = os.getenv('SIEMATIC_TLS_ENABLED', '').strip().lower() in {'1', 'true', 'yes', 'on'}; scheme = 'https' if tls_enabled else 'http'; context = ssl.create_default_context(cafile=os.getenv('CHERRYPY_SSL_CERT')) if tls_enabled else None; response = urllib.request.urlopen(f'{scheme}://127.0.0.1:8000/accounts/login/', timeout=5, context=context); sys.exit(0 if response.status < 500 else 1)"]
